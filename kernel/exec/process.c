...
static void stack_write(uint64_t uva, const void* src, uint64_t len,
                        uint64_t* phys_arr, uint64_t stack_base)
{
...
static void stack_write_u64(uint64_t uva, uint64_t val,
                            uint64_t* phys_arr, uint64_t stack_base)
{
                          ...
                          const char* const* argv, const char* const* envp)
{
    ...
    if (argc > STACK_MAX_ARGS || envc > STACK_MAX_ARGS)
        return 0;
    ...
    for (int i = envc - 1; i >= 0; i--)
    {
        ...
        sp -= len;
        sp &= ~(uint64_t) 7;
        if (sp < stack_base) { kfree(env_uva); kfree(arg_uva); return 0; }
        env_uva[i] = sp;
    ...
    uint64_t wp = sp;
    stack_write_u64(wp, (uint64_t) argc, phys, stack_base); wp += 8;
    for (int i = 0; i < argc; i++) { stack_write_u64(wp, arg_uva[i], phys, stack_base); wp += 8; }
    stack_write_u64(wp, 0, phys, stack_base); wp += 8;
    for (int i = 0; i < envc; i++) { stack_write_u64(wp, env_uva[i], phys, stack_base); wp += 8; }
    stack_write_u64(wp, 0, phys, stack_base); wp += 8;
