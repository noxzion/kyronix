...
static void mouse_irq(int irq, void* arg)
{
    ...
    if (dx)
        input_push(INPUT_DEV_MOUSE, EV_REL, REL_X, dx);
    if (dy)
        input_push(INPUT_DEV_MOUSE, EV_REL, REL_Y, dy);

    uint8_t btns = flags & 0x07;
    uint8_t changed = btns ^ g_btns_prev;
    g_btns_prev = btns;
    if (changed & 1)
        input_push(INPUT_DEV_MOUSE, EV_KEY, BTN_LEFT, (btns & 1) ? 1 : 0);
    if (changed & 2)
        input_push(INPUT_DEV_MOUSE, EV_KEY, BTN_RIGHT, (btns & 2) ? 1 : 0);
    if (changed & 4)
        input_push(INPUT_DEV_MOUSE, EV_KEY, BTN_MIDDLE, (btns & 4) ? 1 : 0);
