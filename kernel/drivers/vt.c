...
static int64_t vt_ioctl(vfs_node_t* n, uint64_t req, uint64_t arg)
{
    ...
    case KDGKBMODE:
        if (arg && !uptr_ok_w((void*) (uintptr_t) arg, sizeof(int)))
            return -14;
        if (arg)
            *(int*) (uintptr_t) arg = g_kb_mode;
        return 0;
    case KDSKBMODE:
        g_kb_mode = (int) arg;
        return 0;
    case KDGKBTYPE:
        if (arg && !uptr_ok_w((void*) (uintptr_t) arg, sizeof(char)))
            return -14;
        if (arg)
            *(char*) (uintptr_t) arg = (char) KB_101;
        return 0;
    case KDMAPDISP:
    case KDUNMAPDISP:
    case GIO_SCRNMAP:
    ...
    case TIOCGPGRP:
        if (arg && !uptr_ok_w((void*) (uintptr_t) arg, sizeof(int)))
            return -14;
        if (arg)
            *(int*) (uintptr_t) arg = tty_get_fg_pgid();
        return 0;
    case TIOCSPGRP:
        if (arg && !uptr_ok((void*) (uintptr_t) arg, sizeof(int)))
            return -14;
        if (arg)
            tty_set_fg_pgid(*(int*) (uintptr_t) arg);
        return 0;
    case TIOCSCTTY:
    case TIOCNOTTY:
    case TIOCEXCL:
