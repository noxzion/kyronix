...
typedef struct { char mode, waitv; short relsig, acqsig, frsig; } vt_mode_t;
...
static int64_t vt_ioctl(vfs_node_t* n, uint64_t req, uint64_t arg)
{
    ...
    case VT_SETMODE: case VT_RELDISP:
    case VT_ACTIVATE: case VT_WAITACTIVE:
    case VT_DISALLOCATE: case VT_SETSCRNMAP:
        return 0;
    ...
    case TCGETS: {
        ...
        t->c_cflag = ts.c_cflag; t->c_lflag = ts.c_lflag;
