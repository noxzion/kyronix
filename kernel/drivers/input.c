...
void input_push(int dev, uint16_t type, uint16_t code, int32_t value)
{
    ...
    e->buf[e->head] = (input_event_t){
        .sec = g_ticks / 1000, .usec = (g_ticks % 1000) * 1000,
...
{
    return (req >> 30) == 2 && ((req >> 8) & 0xFF) == 0x45 &&
           (req & 0xFF) == nr;
}
...
static int64_t evdev_ioctl(vfs_node_t* n, uint64_t req64, uint64_t arg)
{
    ...
    if ((req >> 30) == 2 && ((req >> 8) & 0xFF) == 0x45 &&
        (req & 0xFF) >= 0x20 && (req & 0xFF) < 0x40) {
