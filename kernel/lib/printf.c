...
static void emit_str(printf_putchar_fn fn, void *ctx, const char *s,
                     int width, bool left, int *count) {
    int len = (int)strlen(s);
    if (!left) for (int i = len; i < width; i++) emit(fn, ctx, ' ', count);
    while (*s) emit(fn, ctx, *s++, count);
    if (left)  for (int i = len; i < width; i++) emit(fn, ctx, ' ', count);
}

static void emit_uint(printf_putchar_fn fn, void *ctx,
                      uint64_t val, int base, bool upper,
    ...
    char pad = zero_pad ? '0' : ' ';
    if (!left) for (int i = len; i < width; i++) emit(fn, ctx, pad, count);
...
int vprintf_cb(printf_putchar_fn fn, void *ctx, const char *fmt, va_list ap) {
    ...
    while (*fmt) {
        ...
        switch (*fmt++) {
            case 'c': emit(fn, ctx, (char)va_arg(ap, int), &count); break;
            case 's': {
            ...
            case 'd': case 'i': {
                ...
                if (v < 0) { emit(fn, ctx, '-', &count); v = -v; }
                emit_uint(fn, ctx, (uint64_t)v, 10, false, width, left, zero_pad, &count);
                break;
            }
            case 'u':
                emit_uint(fn, ctx,
                          is_long ? va_arg(ap, uint64_t) : va_arg(ap, unsigned),
                          10, false, width, left, zero_pad, &count);
                break;
            case 'x':
                emit_uint(fn, ctx,
                          is_long ? va_arg(ap, uint64_t) : va_arg(ap, unsigned),
                          16, false, width, left, zero_pad, &count);
                break;
            case 'X':
                emit_uint(fn, ctx,
                          is_long ? va_arg(ap, uint64_t) : va_arg(ap, unsigned),
                          16, true, width, left, zero_pad, &count);
                break;
            case 'o':
                emit_uint(fn, ctx,
                          is_long ? va_arg(ap, uint64_t) : va_arg(ap, unsigned),
                          8, false, width, left, zero_pad, &count);
                break;
            case 'p': {
            ...
            case '%': emit(fn, ctx, '%', &count); break;
            default:  emit(fn, ctx, '?', &count); break;
        }
    }
    return count;
}
