...
{
    kx_prog = "sed";
    if (argc < 2) kx_die("usage: sed 's/OLD/NEW/[g]' [FILE...]");
    const char *expr = argv[1];
    if (strncmp(expr, "s/", 2) != 0) kx_die("only s/OLD/NEW/[g] supported");
    char buf[512];
    snprintf(buf, sizeof(buf), "%s", expr + 2);
    char *old = buf;
    char *slash = strchr(old, '/');
    if (!slash) kx_die("bad expression");
    *slash++ = 0;
    char *new = slash;
    slash = strchr(new, '/');
    if (!slash) kx_die("bad expression");
    *slash++ = 0;
    ...
    for (int i = first; i < argc; i++) {
        ...
        if (!f) { kx_warn(argv[i]); rc = 1; continue; }
